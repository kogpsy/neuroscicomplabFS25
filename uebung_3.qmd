...
- Ein File
- glimpse um zu kontrollieren ob datensatz ok
- ab #--- wird es dann gestrippt
- Rmd kommt dann bei analyse
...
- Laden Sie das _R-Project_ [uebung-3](../../downloadable_files/uebung_3.zip) herunter und entzippen Sie den Ordner. Im `data`-Ordner finden Sie die Einzeldateien vom Experiment.

- Erstellen Sie ein neues `.R`-File und speichern Sie dieses unter `preprocessing_randomdot.Rmd` im Projekt-Ordner. Falls Sie in einer Gruppe arbeiten speichern Sie dieses mit den Initialen ab, z.B. preprocessing_randomdot_initialen_initialen_initialen.R`.
...
```{r echo=FALSE}
d <- read.csv("data/dataset_rdk_clean.csv")
glimpse(d)
```
- __Datensatz speichern__: Speichern Sie den neuen Datensatz (der jetzt __alle__ Datensätze vorverarbeitet und zusammengefügt enthält) als `.csv`-File namens `dataset_rdk_clean_initialen.csv`.
...
Laden Sie folgende Dateien unter `Übung 3` auf Ilias hoch:
    ...
    - `dataset_rdk_clean_initialen.csv`-File
